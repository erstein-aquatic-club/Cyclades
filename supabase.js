(function () {
  ...
  function loginEmail(username){var value=String(username||"").trim().toLowerCase();return value.indexOf("@")>=0?value:value+"@cyclades.local";}
  ...
  async function signIn(username,password){var sb=getClient();if(!sb)throw new Error("Supabase n’est pas encore configuré.");var r=await sb.auth.signInWithPassword({email:loginEmail(username),password:password});if(r.error)throw r.error;return r.data;}
  async function updatePassword(password){var sb=getClient();if(!sb)throw new Error("Supabase n’est pas encore configuré.");if(String(password||"").length<6)throw new Error("Le mot de passe doit contenir au moins 6 caractères.");var r=await sb.auth.updateUser({password:password});if(r.error)throw r.error;return r.data;}
  ...
  window.CycladesBackend={configured:configured,getClient:getClient,getSession:getSession,signIn:signIn,updatePassword:updatePassword,signOut:signOut,listExpenses:listExpenses,addExpense:addExpense,deleteExpense:deleteExpense};
})();
