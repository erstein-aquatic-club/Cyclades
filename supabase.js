(function () {
  ...
  async function listExpenses(){var sb=getClient(),t=token();if(!sb||!t)throw new Error("Connexion requise.");var r=await sb.rpc("app_list_expenses_v2",{p_token:t});if(r.error)throw r.error;return r.data||[];}
  async function addExpense(expense){var sb=getClient(),t=token();if(!sb||!t)throw new Error("Connexion requise.");var people=(expense.people&&expense.people.length)?expense.people:["Francois","Onja"];var r=await sb.rpc("app_add_expense_v2",{p_token:t,p_amount:Number(expense.amount),p_currency:expense.currency||"EUR",p_category:expense.category||"Autre",p_description:expense.description||"",p_expense_date:expense.expense_date,p_island:expense.island||"",p_payment_method:expense.payment_method||"",p_people:people});if(r.error)throw r.error;return r.data;}
  async function setExpensePeople(id,people){var sb=getClient(),t=token();if(!sb||!t)throw new Error("Connexion requise.");if(!people||!people.length)throw new Error("Choisis au moins une personne.");var r=await sb.rpc("app_set_expense_people",{p_token:t,p_expense_id:id,p_people:people});if(r.error)throw r.error;return r.data;}
  ...
  window.CycladesBackend={configured:configured,getClient:getClient,getSession:getSession,signIn:signIn,signOut:signOut,updatePassword:updatePassword,listExpenses:listExpenses,addExpense:addExpense,setExpensePeople:setExpensePeople,deleteExpense:deleteExpense};
})();
