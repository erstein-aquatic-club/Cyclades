...
returns table (id uuid,name text,slug text,destination_label text,start_date date,end_date date,timezone text,currency text,cover_image_url text,status text,role text,member_count bigint)
language plpgsql security definer set search_path='public','extensions' as $$
declare v_user_id uuid;
begin
  select s.user_id into v_user_id from public.app_sessions s where s.token_hash=encode(digest(p_token,'sha256'),'hex') and s.expires_at>now() limit 1;
  if v_user_id is null then raise exception 'Session invalide'; end if;
  return query select t.id,t.name,t.slug,t.destination_label,t.start_date,t.end_date,t.timezone,t.currency,t.cover_image_url,t.status,tm.role,
    (select count(*) from public.trip_members tm2 where tm2.trip_id=t.id)
  from public.trips t join public.trip_members tm on tm.trip_id=t.id and tm.user_id=v_user_id
  order by t.start_date desc,t.created_at desc;
end; $$;

create or replace function public.app_get_trip(p_token text,p_trip_id uuid)
returns table (id uuid,name text,slug text,destination_label text,start_date date,end_date date,timezone text,currency text,cover_image_url text,status text,role text)
language plpgsql security definer set search_path='public','extensions' as $$
declare v_user_id uuid;
begin
  select s.user_id into v_user_id from public.app_sessions s where s.token_hash=encode(digest(p_token,'sha256'),'hex') and s.expires_at>now() limit 1;
  if v_user_id is null then raise exception 'Session invalide'; end if;
  return query select t.id,t.name,t.slug,t.destination_label,t.start_date,t.end_date,t.timezone,t.currency,t.cover_image_url,t.status,tm.role
  from public.trips t join public.trip_members tm on tm.trip_id=t.id
...
begin
  select s.user_id into v_user_id from public.app_sessions s where s.token_hash=encode(digest(p_token,'sha256'),'hex') and s.expires_at>now() limit 1;
  if v_user_id is null then raise exception 'Session invalide'; end if;
  if not exists(select 1 from public.trip_members tm where tm.trip_id=p_trip_id and tm.user_id=v_user_id) then raise exception 'Accès au voyage refusé'; end if;
  return query select tm.id,u.id,u.username,u.display_name,tm.role,tm.joined_at
  from public.trip_members tm join public.app_users u on u.id=tm.user_id where tm.trip_id=p_trip_id
...
returns table (id uuid,user_id uuid,amount numeric,currency text,category text,description text,expense_date date,island text,payment_method text,created_at timestamptz,updated_at timestamptz,trip_id uuid,people text[])
language plpgsql security definer set search_path='public','extensions' as $$
declare v_user_id uuid;
begin
  select s.user_id into v_user_id from public.app_sessions s where s.token_hash=encode(digest(p_token,'sha256'),'hex') and s.expires_at>now() limit 1;
  if v_user_id is null then raise exception 'Session invalide'; end if;
  if not exists(select 1 from public.trip_members tm where tm.trip_id=p_trip_id and tm.user_id=v_user_id) then raise exception 'Accès au voyage refusé'; end if;
  return query select e.id,e.user_id,e.amount,e.currency,e.category,e.description,e.expense_date,e.island,e.payment_method,e.created_at,e.updated_at,e.trip_id,
    coalesce(array_agg(ep.person_name order by ep.person_name) filter (where ep.person_name is not null),array[]::text[])
  from public.expenses e left join public.expense_people ep on ep.expense_id=e.id where e.trip_id=p_trip_id
  group by e.id order by e.expense_date desc,e.created_at desc;
end; $$;

create or replace function public.app_add_trip_expense_v3(p_token text,p_trip_id uuid,p_amount numeric,p_currency text,p_category text,p_description text,p_expense_date date,p_island text,p_payment_method text,p_people text[])
...
begin
  ...
  if exists(select 1 from unnest(p_people) p(person_name) where not exists(select 1 from public.trip_members tm join public.app_users u on u.id=tm.user_id where tm.trip_id=p_trip_id and lower(u.username)=lower(p.person_name))) then raise exception 'Personne invalide pour ce voyage'; end if;
  insert into public.expenses(user_id,trip_id,amount,currency,category,description,expense_date,island,payment_method)
  values(v_user_id,p_trip_id,p_amount,coalesce(p_currency,'EUR'),coalesce(p_category,'Autre'),nullif(p_description,''),p_expense_date,nullif(p_island,''),nullif(p_payment_method,'')) returning id into v_expense_id;
  foreach v_person in array p_people loop
    insert into public.expense_people(expense_id,person_name)
    select v_expense_id,u.username from public.trip_members tm join public.app_users u on u.id=tm.user_id where tm.trip_id=p_trip_id and lower(u.username)=lower(v_person) on conflict do nothing;
  end loop;
  return v_expense_id;
end; $$;
...
begin
  ...
  foreach v_person in array p_people loop
    insert into public.expense_people(expense_id,person_name)
    select p_expense_id,u.username from public.trip_members tm join public.app_users u on u.id=tm.user_id where tm.trip_id=p_trip_id and lower(u.username)=lower(v_person) on conflict do nothing;
  end loop;
  return true;
end; $$;
...
-- The custom-auth transition requires SECURITY DEFINER because tables have no anon grants.
-- Revoke the default PUBLIC privilege and expose only the explicit RPC surface to anon.
